...
const m_consonant = new newHashMap();
m_consonant.put("k", "\u0f40");
...
// subjoined letters
const m_subjoined = new newHashMap();
m_subjoined.put("k", "\u0f90");
...
const m_final_uni = new newHashMap();
m_final_uni.put("M", "\u0f7e");
m_final_uni.put("~M`", "\u0f82");
m_final_uni.put("~M", "\u0f83");
m_final_uni.put("X", "\u0f37");
...
const m_final_class = new newHashMap();
...
const m_special = new newHashSet();
m_special.add(".");
m_special.add("+");
m_special.add("-");
m_special.add("~");
...
const m_superscripts = new newHashMap();
...
const m_subscripts = new newHashMap();
tmpSet = new newHashSet();
tmpSet.add("k");
tmpSet.add("kh");
tmpSet.add("g");
...
const m_prefixes = new newHashMap();
tmpSet = new newHashSet();
tmpSet.add("c");
tmpSet.add("ny");
tmpSet.add("t");
...
const m_suffixes = new newHashSet();
m_suffixes.add("'");
m_suffixes.add("\u2018");
m_suffixes.add("\u2019");
m_suffixes.add("g");
...
const m_ambiguous_key = new newHashMap();
...
const m_ambiguous_wylie = new newHashMap();
...
// top letters
const m_tib_top = new newHashMap();
m_tib_top.put('\u0f40', "k");
m_tib_top.put('\u0f41', "kh");
m_tib_top.put('\u0f42', "g");
m_tib_top.put('\u0f43', "g+h");
...
// subjoined letters
const m_tib_subjoined = new newHashMap();
...
const m_tib_vowel = new newHashMap();
m_tib_vowel.put('\u0f71', "A");
m_tib_vowel.put('\u0f72', "i");
m_tib_vowel.put('\u0f73', "I");
m_tib_vowel.put('\u0f74', "u");
...
// long (Skt) vowels
const m_tib_vowel_long = new newHashMap();
m_tib_vowel_long.put("i", "I");
m_tib_vowel_long.put("u", "U");
m_tib_vowel_long.put("-i", "-I");

// final symbols => wylie
const m_tib_final_wylie = new newHashMap();
...
const m_tib_final_class = new newHashMap();
...
const m_tib_caret = new newHashMap();
m_tib_caret.put("ph", "f");
m_tib_caret.put("b", "v");

// other stand-alone characters
const m_tib_other = new newHashMap();
m_tib_other.put(' ', "_");
m_tib_other.put('\u0f04', "@");
m_tib_other.put('\u0f05', "#");
m_tib_other.put('\u0f06', "$");
...
const m_tib_stacks = new newHashSet();
...
const m_tokens_start = new newHashMap();
...
const State = { PREFIX: 0, MAIN: 1, SUFF1: 2, SUFF2: 3, NONE: 4 }
// split a string into Wylie tokens;
// make sure there is room for at least one null element at the end of the array
const splitIntoTokens = function(str) {
  const tokens = [] // size = str.length + 2
  let i = 0;
  const maxlen = str.length;
  TOKEN:
  while (i < maxlen) {
    const c = str.charAt(i);
    const mlo = m_tokens_start.get(c);
    // if there are multi-char tokens starting with this char, try them
    if (mlo != null) {
      for (let len = mlo; len > 1; len--) {
        if (i <= maxlen - len) {
          const tr = str.substring(i, i + len);
...
const subjoined = function(s) { return m_subjoined.get(s) }
const vowel = function(s) { return m_vowel.get(s) }
const final_uni = function(s) { return m_final_uni.get(s) }
const final_class = function(s) { return m_final_class.get(s) }
const other = function(s) { return m_other.get(s) }
const isSpecial = function(s) { return m_special.contains(s) }
const isSuperscript = function(s) { return m_superscripts.containsKey(s) }
const superscript = function(sup, below) {
  const tmpSet = m_superscripts.get(sup);
  if (tmpSet == null) return false;
  return tmpSet.contains(below);
}
const isSubscript = function(s) { return m_subscripts.containsKey(s) }
const subscript = function(sub, above) {
  const tmpSet = m_subscripts.get(sub);
  if (tmpSet == null) return false;
  return tmpSet.contains(above);
}
const isPrefix = function(s) { return m_prefixes.containsKey(s) }
const prefix = function(pref, after) {
  const tmpSet = m_prefixes.get(pref);
  if (tmpSet == null) return false;
  return tmpSet.contains(after);
}
const isSuffix = function(s) { return m_suffixes.contains(s) }
const isSuff2 = function(s) { return m_suff2.containsKey(s) }
const suff2 = function(suff, before) {
  const tmpSet = m_suff2.get(suff);
  if (tmpSet == null) return false;
  return tmpSet.contains(before);
}
const ambiguous_key = function(syll) { return m_ambiguous_key.get(syll) }
const ambiguous_wylie = function(syll) { return m_ambiguous_wylie.get(syll) }
const tib_top = function(c) { return m_tib_top.get(c) }
const tib_subjoined = function(c) { return m_tib_subjoined.get(c) }
const tib_vowel = function(c) { return m_tib_vowel.get(c) }
const tib_vowel_long = function(s) { return m_tib_vowel_long.get(s) }
const tib_final_wylie = function(c) { return m_tib_final_wylie.get(c) }
const tib_final_class = function(c) { return m_tib_final_class.get(c) }
const tib_caret = function(s) { return m_tib_caret.get(s) }
const tib_other = function(c) { return m_tib_other.get(c) }
const tib_stack = function(s) { return m_tib_stacks.contains(s) }
...
function fromWylieOneTsekbar(tokens, i, opts) { // (str, int)
  ...
  if (opts.check && warns.length == 0 && check_root && root_idx >= 0) {
    ...
    if (consonants.length == 2 && root_idx != 0 &&
      ...
      const cc = consonants.join("");
      const expect_key = ambiguous_key(cc);
...
function fromWylieOneStack(tokens, i, opts) {
  ...
  let vowel_sign = null; // any vowel signs (that go under or above the main stack)
  let single_consonant = null; // did we find just a single consonant?
  let plus = false; // any explicit subjoining via '+'?
  let caret = 0; // find any '^'?
  const final_found = new newHashMap(); // keep track of finals (H, M, etc) by class
  ...
  if (t2 != null && isSuperscript(t) && superscript(t, t2)) {
    if (opts.check_strict) {
      let next = consonantString(tokens, i + 1);
      ...
      for (let z = 0; z < 2; z++) {
        ...
        if (t2 != null && isSubscript(t2)) {
          ...
          if (opts.check_strict && !plus) {
            let prev = consonantStringBackwards(tokens, i - 1, orig_i);
  ...
  while (t != null && final_class(t) != null) {
    const uni = final_uni(t);
    const klass = final_class(t);
...
function fromWylie(str, opts, warns) {
  ...
  let out = '', line = 1, units = 0, i = 0;
  const tokens = splitIntoTokens(str);
  ITER:
  while (tokens[i] != null) {
    let t = tokens[i], o = null
    ...
    if (vowel(t) != null || consonant(t) != null) {
      const tb = fromWylieOneTsekbar(tokens, i, opts);
      let word = '';
      ...
      i += tb.tokens_used;
      units++;
      for (let w = 0; w < tb.warns.length; w++) {
...
function formatHex(t) { // char
  ...
  let sb = '';
  sb += '\\u';
  const s = t.charCodeAt(0).toString(16);
  for (let i = s.length; i < 4; i++) sb += '0';
  sb += s;
  return sb;
}
...
function toWylieOneTsekbar(str, len, i) {
  const orig_i = i;
  let warns = [];
  const stacks = [];// ArrayList<ToWylieStack>;
  ITER:
  while (true) {
    const st = toWylieOneStack(str, len, i);
    stacks.push(st);
  ...
  if (stacks.length > 1 && stacks[0].single_cons != null) {
    ...
    const cs = stacks[1].cons_str.replace(/\+w/g, "")
  ...
  if (stacks[0].prefix && tib_stack(stacks[0].single_cons + "+" + stacks[1].cons_str)) { stacks[0].dot = true; }
  // put it all together
  let out = ''
  for (var si = 0; si < stacks.length; si++) out += putStackTogether(stacks[si])
  const ret = new ToWylieTsekbar();
  ret.wylie = out;
  ret.tokens_used = i - orig_i;
  ret.warns = warns;
  return ret;
...
function toWylie(str, escape, warns) {
  ...
  while (i < len) {
    ...
    if (tib_top(t) != null) {
      const tb = toWylieOneTsekbar(str, len, i);
      out += tb.wylie;
      i += tb.tokens_used;
      units++;
      for (let w = 0; w < tb.warns.length; w++) warnl(warns, line, tb.warns[w]);
